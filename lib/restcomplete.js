...
function buildDocs(){
	new noodles.Template({
		rawString: fs.readFileSync(__dirname + '/index.html'),
		metaData:'',
...
function _buildDocs(template){
	...
	serviceFiles.forEach(function(a,index){
		var service = require('../services/' + a),
			article = '',
			services = service.services,
			subnav = {};
		docs.context.mainNav[service.docPage] = {
		...
		if(index === 0){
			docs.indexPage = service.docPage;
		}
		template.execute({
			mainNav:docs.context.mainNav,
...
function docServiceInit(){
	var uri = root + config.serverOptions.docsURIBase + '/:docpage';
	console.log(uri);
	server['GET'](uri,docProxy);
}
/*--module--
name:docProxy
description:
...
function docProxy(req,res,next){
	var docPage = req.params.docpage;
	console.log(docPage);
};